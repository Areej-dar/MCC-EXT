...
def run_pipeline(input_excel_path: str, output_dir: str):
    ...
    msg = f"Main Sheet Column Names: {df_main.columns.tolist()}"
    print(msg); logs.append(msg)

    whitelist_file_path = "Whitelist 2.0.xlsx" 
    # Load
    df_whitelist = pd.read_excel(whitelist_file_path)
    msg = f"Whitelist Sheet Column Names: {df_whitelist.columns.tolist()}"
    print(msg); logs.append(msg)
    ...
    msg = f"Filtered DataFrame shape: {filtered_df.shape}"
    print(msg); logs.append(msg)

    #Inspect and clean the DE6 CDHLDR AMT column
    
    msg = f"Raw DE6 CDHLDR AMT values:\n{filtered_df['DE6 CDHLDR AMT '].head(20)}"
    print(msg)
    logs.append(msg)
    ...
    msg = f"Cleaned DE6 CDHLDR AMT values:\n{filtered_df['DE6 CDHLDR AMT '].head(20)}"
    print(msg); logs.append(msg)
    ...
    msg = f"Grouped DataFrame shape: {grouped_df.shape}"
    print(msg); logs.append(msg)

    msg = f"Sample grouped data:\n{grouped_df.head()}"
    print(msg); logs.append(msg)
    ...
    msg = f"Final DataFrame shape: {final_df.shape}"
    print(msg); logs.append(msg)

    msg = f"Sample final data:\n{final_df.head()}"
    print(msg); logs.append(msg)
    ...
    base_merchants = [
        ...
        "kitopi", "12go nicosia cyp", "1688com", "abstractmag", "aeon", "ahrefs", "amazon", "airasia", "trackdrive", "bbfunded", "bc basecamp",
        "bindawood", "bin dawood", "bath and body works", "buzzoid", "carrefoursa", "crown vps", "ding", "dingcom", "dunelm", "esgcv", "flixbus",
        "fixify", "fanatical", "heroku", "theitin", "koton", "klook travel", "lcwaikiki", "lovable", "manning", "matw project", "maventrading", 
        "micahguru formations", "nodemaven", "oculus", "onlyfanscom", "pskoolcom", "param", "parcelhero", "payfast", "postmarkappcom", "pt finnet",
        "resend", "restoran", "SGT pdfhousecom", "starbucks", "stealthwriter", "sephora", "tamm", "taxif", "temucom", "tencent", "unlimited gbr", 
        "us polo", "majorcineplex", "foodpanda", "food panda", "talabat", "2co", "arabian oud", "spylix", "al nahdi", "bim", "borderpass",
        "cdkeyscom", "cjdropshipping", "deploy saasgroup", "discord", "dropbox", "ecitizen", "esnekpos", "everhour subscription", "globale",
        "i ticket baku aze", "jernmate", "malayan bank berhad", "mobily", "nefturk telecom", "quakrio", "shein", "smart craft vientiane cap lao",
        "splice", "taj the grocer", "tap ftmarkets dubai are", "teachable", "track taco", "wwwf1com", "yoco", "fiverr"
    ]
    ...
    if not removed_base.empty:
        msg = f"Removed due to base match:\n{removed_base[['DE43', 'DE6(SUMMED)', 'Status']]}"
        print(msg); logs.append(msg)
        removed_base.to_excel(os.path.join(output_dir, 'removed_base_merchants.xlsx'), index=False)
    else:
        msg = "No base merchants matched."
        print(msg); logs.append(msg)
    ...
    output_file_path = os.path.join(output_dir, 'cleaned.xlsx')
    result_df.to_excel(output_file_path, index=False)
    msg = f"Final cleaned data saved to {output_file_path}"
    print(msg); logs.append(msg)
    ...
    def generate_risk_assessment_google(name, country):
        ...
        if detected:
            msg = f"[Sensitive content detected]: {', '.join(detected)}"
            print(msg); logs.append(msg)
    ...
    for start_idx in range(0, len(df), chunk_size):
        ...
        for idx, row in chunk.iterrows():
            ...
            if "to be blacklisted" in status:
                msg = f"⏭️ Row {idx} skipped — already blacklisted."
                print(msg); logs.append(msg)
                skipped_count += 1
                continue
            ...
            if mcc in WHITELISTED_MCC:
                msg = " → Whitelisted, skipping detailed risk check.\n"
                print(msg); logs.append(msg)
                ...
                msg = f"Risk Level: {lvl} | Score: {sc} | Confidence: {conf}"
                print(msg); logs.append(msg)

                msg = f"Summary   : {summ_txt}"
                print(msg); logs.append(msg)
    ...
    report_file = os.path.join(output_dir, 'risk_report.xlsx')
    ...
    msg = f"Processed merchants: {processed_count}"
    print(msg); logs.append(msg)

    msg = f"Skipped merchants  : {skipped_count}"
    print(msg); logs.append(msg)

    return output_file_path, report_file, logs